...
#import "UIView+RITLBadge.h"
#import <objc/runtime.h>


/// object ? object : subObject
static inline id ritl_badge_default(id object,id subObject)
{
...
+ (void)load
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL originSel = @selector(layoutSubviews);
        SEL swizzledSel = @selector(ritl_layoutSubviews);
        
        Method originMethod = class_getInstanceMethod(self, originSel);
        Method swizzledMethod = class_getInstanceMethod(self, swizzledSel);
    
        BOOL didAddMethod = class_addMethod(self, originSel, method_getImplementation(swizzledMethod), method_getTypeEncoding(swizzledMethod));
...
- (void)ritl_layoutSubviews
{
    ...
    //进行布局
    CGFloat height = self.ritl_badgeSize.height;
    CGFloat width = self.ritl_badgeSize.width;
    CGFloat originY = self.ritl_badgeInset.top;
    CGFloat originX = self.bounds.size.width - width / 2.0 - self.ritl_badgeInset.right;
    self.ritl_badgeLabel.frame = CGRectMake(originX, originY, width, height);
    self.ritl_badgeLabel.layer.cornerRadius = MIN(height, width) / 2.0;
}
...
{
    return ritl_badge_default(objc_getAssociatedObject(self, _cmd),[UIColor colorWithRed:(255.0 / 255.0) green:(85.0 / 255) blue:(85.0 / 255) alpha:1]);
}
...
{
    return ritl_badge_default(objc_getAssociatedObject(self, _cmd),UIColor.whiteColor);
}
...
{
    return ritl_badge_default(objc_getAssociatedObject(self, _cmd),[UIFont systemFontOfSize:10]);
}
...
{
    return ritl_badge_default(objc_getAssociatedObject(self, _cmd),self.ritl_badgeTextFont);
}
